    ...
    void read_passengers_file() {
        ...
        while (getline(infile, line)) {
            ...
            if (pos != string::npos) {
                ...
                string passenger_name = line.substr(pos + 2); // skip ": "
                
                // Find the seat and reserve it if it exists
                for (int i = 0; i < 13; i++) {
                    for (int j = 0; j < 4; j++) {
        ...
        switch (choice) {
            ...
            case 2: {
                ...
                if (flight.is_valid_seat(row, col) && flight.is_seat_empty(row, col)) {
                    ...
                    cout << "Invalid seat position or seat is already reserved. Try another seat." << endl;
                }
                break;
            }
            case 3: {
                ...
                if (flight.is_valid_seat(row, col) && !flight.is_seat_empty(row, col)) {
                    ...
                    cout << "Invalid seat position or seat is already empty. Try another seat." << endl;
                }
                break;
            }
            case 4:
