...
void menu() {
    ...
    void addRecord(Car cars[], int& size) {
        if (size >= MAXSIZE) {
            cout << "Cannot add more records. The maximum size has been reached." << endl;
            return;
        }

        cars[size].id = new char[50];
        cars[size].make = new char[50];
        cars[size].model = new char[50];
        cars[size].color = new char[50];
